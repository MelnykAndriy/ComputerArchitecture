...
ukr_letters = [letter for letter in letters if letter not in special_russian_letters]
ukr_letters.extend(special_ukrainian_letters)


def ukrainian_words_from_urls(urls_to_process):
    return reduce(lambda words, url: words + split_into_ukrainian_words(url_text_pieces(url)),
                  urls_to_process,
                  [])


def split_into_ukrainian_words(list_of_text_pieces):
    spliter = re.compile(u"[%s]+" % reduce(lambda x, y: x+y, ukr_letters), re.UNICODE)
